...
namespace dkm {
	...
	*/
	namespace details {
		...
		*/
		template <typename T, size_t N>
		T distance_manhattan(const std::array<T, N>& point_a, const std::array<T, N>& point_b) {
			T d_manhattan = T();
			for (typename std::array<T, N>::size_type i = 0; i < N; ++i) {
				auto delta = std::fabs(point_a[i] - point_b[i]);
				d_manhattan += delta;
			}
			return d_manhattan;
		}
		...
		T distance(const std::array<T, N>& point_a, const std::array<T, N>& point_b) {
			...
			return distance_manhattan(point_a, point_b);
		}
		...
		*/
		...
		std::vector<T> closest_distance(
			...
			//	T closest = distance_squared(d, means[0]);
				T closest = distance_manhattan(d, means[0]);
				for (auto& m : means) {
				//	T distance = distance_squared(d, m);
					T distance = distance_manhattan(d, m);
					if (distance < closest)
						closest = distance;
				}
		...
		*/
		...
		//	T smallest_distance = distance_squared(point, means[0]);
			T smallest_distance = distance_manhattan(point, means[0]);
			typename std::array<T, N>::size_type index = 0;
			T distance;
			for (size_t i = 1; i < means.size(); ++i) {
		//		distance = distance_squared(point, means[i]);
				distance = distance_manhattan(point, means[i]);
