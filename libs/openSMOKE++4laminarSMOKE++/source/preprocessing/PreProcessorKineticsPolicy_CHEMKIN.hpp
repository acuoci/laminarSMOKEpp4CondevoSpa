...
namespace OpenSMOKE
{
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::ReadFromASCIIFile(const std::string file_name)
	{
...

		{
			...
			// Reaction classes
			reaction_classes_ = new ReactionClasses(	myKinetics->reaction_mainclass_names(), myKinetics->reaction_mainclass_lines_abs(),
														myKinetics->endreaction_mainclass_names(), myKinetics->endreaction_mainclass_lines_abs(),
														myKinetics->reaction_subclass_names(), myKinetics->reaction_subclass_lines_abs(),
														myKinetics->endreaction_subclass_names(), myKinetics->endreaction_subclass_lines_abs() );
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::KineticsFromASCIIFile(AtomicCompositionTable& atomicComposition, std::ostream& flog)
	{
		...
		// Reaction classes (if active)
		if (reaction_classes_->is_active() == true)
		{
			for (unsigned int j = 0; j < number_of_reactions; j++)
				reaction_classes_->filter_reaction(j, myKinetics->indices_of_good_lines()[reaction_lines[j]]);
		}
	...
	bool PreProcessorKineticsPolicy_CHEMKIN<Reactions>::WriteXMLFile(std::stringstream& fOutput) const
	{
		...
		// Reaction classes (if any)
		if (reaction_classes_->is_active() == true)
		{
			Eigen::MatrixXd nu(names_species_.size(), reactions_.size());
			nu.setZero();
			for (unsigned int j = 0; j < reactions_.size(); j++)
			{
				for (unsigned int i = 0; i < reactions_[j].reactant_nu_indices().size(); i++)
					nu(reactions_[j].reactant_nu_indices()[i], j) += -reactions_[j].reactant_nu()[i];
				for (unsigned int i = 0; i < reactions_[j].product_nu_indices().size(); i++)
					nu(reactions_[j].product_nu_indices()[i], j) += reactions_[j].product_nu()[i];
			}

			reaction_classes_->WriteXMLFile(fOutput, names_species_, nu);
			reaction_classes_->Summary(std::cout);
		}
