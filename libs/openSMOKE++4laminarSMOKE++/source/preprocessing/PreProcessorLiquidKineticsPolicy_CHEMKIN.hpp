...
namespace OpenSMOKE
{
	...
	PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::PreProcessorLiquidKineticsPolicy_CHEMKIN() {
	}

	template<typename Reactions>
	PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::PreProcessorLiquidKineticsPolicy_CHEMKIN(const PreProcessorLiquidKineticsPolicy_CHEMKIN& orig) {
	}
	...
	template<typename Reactions>
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::ReadFromASCIIFile(const std::string file_name)
	{
		...
		liquid_species_.resize(number_of_materials_);
		liquid_species_indices_.resize(number_of_materials_);
		for (unsigned int k=0;k<number_of_materials_;k++)
		{
			std::vector<unsigned int> iLiquidLines;
			...
			for (unsigned int j=iMaterialLines[k]+1;j<iMaterialLines[k+1];j++)
			{
				...
				boost::replace_all(line, "LIQUID", "LIQUID ");
				boost::replace_all(line, "liquid", "LIQUID ");
				...
				{
					size_t found=line.find("LIQUID");
					if (found!=std::string::npos)	iLiquidLines.push_back(j);
				}
				{
			...
			if (iLiquidLines.size() == 0)
			{
				std::cout << "Reading liquid kinetic scheme. No LIQUID section found!" << std::endl;
				return false;
			}
			...
			if (iReactionLines[k][0] <= iLiquidLines[iLiquidLines.size()-1])
			{
				std::cout << "Reading surface kinetic scheme. The REACTIONS section must be defined only after all the SITE sections" << std::endl;
				return false;
			}

			std::vector<std::string> liquid_section(iLiquidLines.size());
			for (unsigned int j=0;j<iLiquidLines.size();j++)
			{
				unsigned int end_line_index = iReactionLines[k][0];
				if (j!=iLiquidLines.size()-1)
					end_line_index = iLiquidLines[j+1];

				for (unsigned int i=iLiquidLines[j];i<end_line_index;i++)
					liquid_section[j] += myKinetics->good_lines()[i] + " ";
			}

			// Liquids
			{
				//liquid_species[k].resize(iLiquidLines.size());
				for (unsigned int j=0;j<iLiquidLines.size();j++)
				{
					std::string line = liquid_section[j];
					...
					for(;;)
					{
						...
						{
							liquid_species_[k].push_back( separate_elements[i] );
							i++;
						}
				...
				for(unsigned int ii=0;ii<liquid_species_[k].size();ii++)
				{
					const std::size_t n = std::count (liquid_species_[k].begin(), liquid_species_[k].end(), liquid_species_[k][ii]);
					if (n>1)
					{
							std::cout << "The following liquid species was specified more than once in the same liquid material: " << liquid_species_[k][ii] << std::endl;
							return false;
					}
				}
				
				
				for(unsigned int i=0;i<liquid_species_[k].size();i++)
				{
					bool iFound = false;
					for(unsigned int ii=0;ii<names_liquid_species_.size();ii++)
						if (liquid_species_[k][i] == names_liquid_species_[ii])
						{
							iFound = true;
							break;
						}
					...
					if (iFound == false)
					{
						names_liquid_species_.push_back(liquid_species_[k][i]);
					}
				}

				liquid_species_indices_[k].resize(liquid_species_[k].size());
				for(unsigned int i=0;i<liquid_species_[k].size();i++)
					for(unsigned int ii=0;ii<names_liquid_species_.size();ii++)
						if (liquid_species_[k][i] == names_liquid_species_[ii])
							liquid_species_indices_[k][i] = ii+1;
			}
		}
	...
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::KineticsFromASCIIFile(	AtomicCompositionTable& atomicComposition, const PreProcessor& preprocessor_kinetics, std::ostream& flog)
	{
		// Populate the maps
		{
			// Populate the map
			{
				...
				for(unsigned int i=0;i<names_liquid_species_.size();i++)
					map_of_species.insert(std::make_pair(names_liquid_species_[i],  count++));
			}
			...
			// Populate the global list
			{
				...
				for(unsigned int i=0;i<names_liquid_species_.size();i++)
					names_species_.push_back(names_liquid_species_[i]);
			}
		...
		for(unsigned int k=0;k<number_of_materials_;k++)
		{
			std::cout << "Reading liquid-phase Material " << k+1 << " over " << number_of_materials_ << std::endl;
			// Reaction units
			...
			for (unsigned int j=0;j<number_of_reactions;j++)
			{
				...
				try
				{
					...
					bool successReading = reactions_[k][j].ReadReactionFromStrings(	list_of_lines, map_of_species, 
																					...
																					boost::lexical_cast<unsigned int>(names_liquid_species_.size()));
	...
	void PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::WriteShortSummaryOnASCIIFile(const std::string file_name, Thermodynamics& thermodynamics) const
	{
		std::cout << " * Writing the summary of liquid-phase kinetic mechanism..." << std::endl;
		...
		const std::size_t number_of_gas_species = names_species_.size() - names_liquid_species_.size();

		fOutput << "---------------------------------------------------------------------------------------" << std::endl;
		fOutput << "                                LIQUID MECHANISM                                       " << std::endl;
		fOutput << "---------------------------------------------------------------------------------------" << std::endl;

		fOutput << std::endl;
		fOutput << std::setw(7) << " "; fOutput << std::left << "Number of materials:       " << number_of_materials_ << std::endl;
		fOutput << std::setw(7) << " "; fOutput << std::left << "Number of liquid species:   " << names_liquid_species_.size() << std::endl;
		...
		if (names_liquid_species_.size() != 0)
		{
			for(unsigned int k=0;k<number_of_materials_;k++)
			{
				fOutput << std::endl;
				...
				fOutput << std::setw(16) << std::left << "liquid-index";
				...
				{
					fOutput << std::setw(7)  << " "; 
					fOutput << std::setw(24) << std::left << liquid_species_[k][i];
					fOutput << std::fixed   << std::setw(16) << std::setprecision(0) << std::left << number_of_gas_species + liquid_species_indices_[k][i];
					fOutput << std::fixed   << std::setw(16) << std::setprecision(0) << std::left << liquid_species_indices_[k][i];
		...
		{
			for (std::vector<ReactionPolicy_Liquid_CHEMKIN>::const_iterator it = reactions_[kk].begin(); it != reactions_[kk].end(); ++it)
			{
...
/*	template<typename T>
	...
	*/
	/*
	template<typename Reactions>
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::WriteASCIIFileOldStyle(const std::string file_name) const
	{
		...
		for (std::vector<ReactionPolicy_CHEMKIN>::const_iterator it = reactions_.begin(); it != reactions_.end(); ++it)
			if ( (*it).IsExplicitlyReversible()==true)
				ErrorMessage("PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::WriteASCIIFileOldStyle(const std::string file_name)",
	...
	*/
	...
	template<typename Reactions>
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::WriteStoichiometricDataOnASCIIFile(std::ostream& fOutput, const unsigned int kk) const	
	{
		{
	...
	template<typename Reactions>
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::WriteReactionOrdersOnASCIIFile(std::ostream& fOutput, const unsigned int kk) const	
	{
	...
	/*
	template<typename Reactions>
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::WriteASCIIFile(const std::string file_name) const
	{
...
*/
	...
	template<typename Reactions>
	bool PreProcessorLiquidKineticsPolicy_CHEMKIN<Reactions>::WriteXMLFile(std::stringstream& fOutput) const
	{
		...
		// Number of liquid species
		fOutput << "<NumberOfLiquidSpecies>" << std::endl;
		fOutput << names_liquid_species_.size() << std::endl;
		fOutput << "</NumberOfLiquidSpecies>" << std::endl;

		// List of site species
		fOutput << "<LiquidSpecies>" << std::endl;
		for(unsigned int i=0;i<names_liquid_species_.size();i++)
			fOutput << names_liquid_species_[i] << std::endl;
		fOutput << "</LiquidSpecies>" << std::endl;
		...
		for(unsigned int k=0;k<number_of_materials_;k++)
		{
			...
			fOutput << liquid_species_[k].size() << std::endl;
			for(unsigned int i=0;i<liquid_species_[k].size();i++)
				fOutput << liquid_species_[k][i] << " " << liquid_species_indices_[k][i] << std::endl;
		...
		for(unsigned int k=0;k<number_of_materials_;k++)
		{
			...
			unsigned int number_of_explicitly_reversible_reactions = 0;
			unsigned int number_of_thermodynamic_reversible_reactions = 0;
			unsigned int number_of_usrprog_reactions = 0;

		
			for (std::vector<ReactionPolicy_Liquid_CHEMKIN>::const_iterator it = reactions_[k].begin(); it != reactions_[k].end(); ++it)
			{
				if ( (*it).IsReversible()==true)
				{
					number_of_reversible_reactions++;
				...
				if ((*it).IsUsrProg() == true)	number_of_usrprog_reactions++;
			}
			...
			std::vector<unsigned int> indices_of_usrprog_reactions(number_of_usrprog_reactions);
			std::vector<std::string> labels_of_usrprog_reactions(number_of_usrprog_reactions);
			...
			{		
				...
				for (std::vector<ReactionPolicy_Liquid_CHEMKIN>::const_iterator it = reactions_[k].begin(); it != reactions_[k].end(); ++it)
				{
					...
					// USRPROG reactions
					if ((*it).IsUsrProg() == true)
					{
						indices_of_usrprog_reactions[count_usrprog] = boost::lexical_cast<int>(it - reactions_[k].begin() + 1);
						labels_of_usrprog_reactions[count_usrprog] = (*it).usrprog_labels()[0];
						count_usrprog++;
					}
				}
				...
				std::vector<int> indices_dummy;
				FormatXML(fOutput, "ThreeBody", indices_dummy);
				
				fOutput << "<FallOff>" << std::endl;
				fOutput << 0 << std::endl;
				FormatXML(fOutput, "Lindemann", indices_dummy);
				FormatXML(fOutput, "Troe", indices_dummy);
				FormatXML(fOutput, "SRI", indices_dummy);
				fOutput << "</FallOff>" << std::endl;
				...
				FormatXML(fOutput, "Chebyshev", indices_dummy);
				FormatXML(fOutput, "PressureLog", indices_dummy);
				FormatXML(fOutput, "ExtendedPressureLog", indices_dummy);
				FormatXML(fOutput, "ExtendedFallOff", indices_dummy);
				FormatXML(fOutput, "FIT1", indices_dummy);
				FormatXML(fOutput, "JanevLanger", indices_dummy);
				FormatXML(fOutput, "LandauTeller", indices_dummy);

				FormatXML(fOutput, "UsrProg", indices_of_usrprog_reactions);
				FormatXML(fOutput, "UsrProg-Labels", labels_of_usrprog_reactions);
