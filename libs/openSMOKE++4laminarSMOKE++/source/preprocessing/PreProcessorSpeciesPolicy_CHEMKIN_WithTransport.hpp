...
//		Omega11jk(T);
	...
	bool PreProcessorSpeciesPolicy_CHEMKIN_WithTransport<Species>::WriteTransportDataOnXMLFile(std::stringstream& xml_string) const
	{
		...
		xml_string << "<Raw-Transport-Parameters>" << std::endl;
		for (unsigned int j = 0; j < this->NC; j++)
			xml_string	<< this->species_[j].shape_factor() << " "
						<< this->species_[j].epsylon_over_kb() << " "
						<< this->species_[j].sigma() << " "
						<< this->species_[j].mu() << " "
						<< this->species_[j].alfa() << " "
						<< this->species_[j].zRot298() << std::endl;
		xml_string << "</Raw-Transport-Parameters>" << std::endl;
