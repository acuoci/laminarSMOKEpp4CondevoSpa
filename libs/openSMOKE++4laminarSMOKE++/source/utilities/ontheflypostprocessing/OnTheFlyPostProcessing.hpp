...
namespace OpenSMOKE
{
	...
	void OnTheFlyPostProcessing::PrepareOutputFiles(const std::vector<std::string>& additional)
	{
		...
		if (class_grouping_ == true)
		{
			...
			for (unsigned int i = 0; i < class_grouping_names_families_.size(); i++)
				widths_of_grouping_names_families_[i] = CalculateSpeciesFieldWidth(class_grouping_names_families_[i] + "_w", static_cast<int>(class_grouping_names_families_.size()));
